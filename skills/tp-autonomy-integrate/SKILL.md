...
## 2. 重新验证 Candidate
Prepare 结果初始是 `NEEDS_VERIFICATION`。由当前 `tp-workflow-orchestrator`/`tp-verification-engineering` 在 integration candidate 上执行必要验证，将非空 evidence 放入该 Integration 的 `evidence/`，再：
