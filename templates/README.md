...
创建新任务时必须写入 `base_version: 5.2.0` 与 `artifact_contract.version: 5.2.0`。模板删减不改变已创建任务目录中的工件路径；历史版本契约（含上一版本）的任务为静态归档，由 Git release 分支承担，`Test-TpSpecTask.ps1` 对其一致拒绝解析。
