...
export function TaskIndex({ rows, available = true, onSelect }: {
    ...
    <div className="task-index-search-row">
      <label htmlFor={`${id}-query`}>搜索任务<Input id={`${id}-query`} type="search" allowClear value={query} onChange={e => { setQuery(e.target.value); setPage(1); }} placeholder="ID、标题或负责人"/></label>
      <p className="muted result-count" role="status">{available ? `匹配 ${filtered.length} / ${rows.length} 项` : '任务数据不可用，未计算匹配数量。'}</p>
    </div>
    <Table<TaskIndexRow> className="task-index-table" size="small" rowKey="task_id" dataSource={filtered}
      pagination={available && filtered.length ? { pageSize: PAGE_SIZE, current: Math.min(page, pageCount), size: 'small', showSizeChanger: false,
        showTotal: (_total, range) => `第 ${range[0]}-${range[1]} 条`, onChange: value => setPage(value) } : false}
