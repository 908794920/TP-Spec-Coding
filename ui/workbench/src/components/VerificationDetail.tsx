...
export function Outcome({ title, value }: { title: string; value: unknown }) {
  ...
  return <Section title={title}>
    ...
    {channel.recorded ? <>
      ...
      <Disclosure label="最新记录的绑定与原始明细"><Fields value={detail}/><p className="muted">历史摘要（不作为 PASS 判断）：{valueText(event.summary)}</p></Disclosure>
    </> : <p className="muted">未取得结构化结果；不使用摘要或其他角色的结论补齐。</p>}
    {records(channel.history).length > 0 && <Disclosure label={`历史记录 · ${valueText(scope.returned)} / ${valueText(scope.total)}`}>
      <p className="muted">最多最近 {valueText(scope.limit)} 条；历史结果不代表当前仍有效。</p>
      {records(channel.history).map((row, i) => <Disclosure key={`${text(row.event_id)}:${i}`} label={`#${text(row.event_id)} · ${text(row.created_at)} · ${text(row.decision)}`}><Fields value={row}/></Disclosure>)}
    </Disclosure>}
  </Section>;
}
export function VerificationDetail({ data }: { data: DetailData }) {
  ...
  return <>
    ...
    <Section title="验收矩阵（工件声明）">
      ...
      {records(acceptance.rows).map((row, i) => <Disclosure key={`${text(row.ac)}:${text(row.line)}:${i}`} label={`${text(row.ac)} · 声明 ${valueText(row.declared_verdict)}`}>
