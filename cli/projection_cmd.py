...
_TYPE_MAP = {
    ...
    "KNOWLEDGE": "KNOWLEDGE",
    # V5.3.3 A-04：reconcile 追加的审计事件；投影为 FACT 保持
    # events.jsonl 合法 type 集合不变（Test-TpSpecTask.ps1 EventTypes 零感知）。
    "RECONCILIATION": "FACT",
}


# V5.3.3 新工件（AI-B 模板定义；AI-C 可继续追加）：存在才纳入 source digest。
_V511_SOURCE_NAMES = (
    "requirement.md",
...
def projection_source_names() -> List[str]:
    """current view source_files 集中注册表（V5.3.3 §10.2）。

    AI-C 接入 V5.3.3 新工件规则时可追加文件名；commit 的
...
def render_projection(conn, task) -> Tuple[str, str, List[str]]:
    ...
    # V5.3.3 单一活动契约：旧契约非终态任务须先经官方 migrate/retire 处理；业务命令不直接在旧契约上重建投影。
    if base_version != active_version():
        raise ValueError(
            f"legacy contract task is a frozen static archive; the V5.3.3 runtime "
...
def validate_projection_files(conn, task, task_dir: Path) -> List[str]:
    ...
    V5.3.3 A-04：reconcile 复用本函数做漂移检测（与 cmd_projection_validate 同逻辑）。
    """
    task_id = task["task_id"]
    status_path = task_dir / "status.yaml"
    events_path = task_dir / "events.jsonl"
