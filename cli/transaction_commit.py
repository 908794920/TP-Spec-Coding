# -*- coding: utf-8 -*-
"""Neutral durable transaction/projection primitives for V5.2.8 Record-first Runtime.
...
def _continuation_sources(task_dir: Path, state: str) -> List[Path]:
    ...
    # V5.2.8 §3.8/§10.2：新工件经集中注册表纳入 source digest（存在才纳入）
...
def _rebuild_current_view_text(task_dir: Path, task, summary: str, flush_id: str) -> str:
    ...
    V5.2.8 intentionally exposes state/phase/result facts, not handoff bureaucracy.
    """
    ...
    body = (
        ...
        f"- 最近执行角色：{owner}\n"
        f"- 最近记录：{summary}\n"
        "\n> V5.2.8：phase 是查询事实，不是流程门禁；继续完成业务工作即可。\n"
    )
...
def _commit_with_recovery(task_dir: Path, conn, rel_paths: List[str], db_and_render: Callable,
    ...
    """一致性提交核心（V5.2.8 durable journal 版）：
