# -*- coding: utf-8 -*-
"""Build read-only snapshots for TP-Spec HTML information cards.

The card layer is presentation-only.  It reads installation/configuration files,
...
def _runtime_health(db_path: Optional[Path]) -> Tuple[str, List[str]]:
    if db_path is None:
...
def build_project_snapshot(
    ...
    *,
    ...
    if db_path:
        ...
        status, issues = _runtime_health(db_path)
...
def build_task_snapshot(
    ...
    *,
    ...
    try:
        conn = dbmod.connect_readonly(str(resolved_db))
        try:
            row = conn.execute(
        ...
        return {
            "card_type": "active_task", "title": "当前任务进度", "generated_at": dbmod.now_iso(), "health": "unavailable",
            "task": {"task_id": task_id0, "title": "", "risk_level": "", "flow_level": "", "state": "", "phase": "", "owner": "", "updated_at": ""},
