...
def cmd_report_cost_benefit(args) -> int:
    """V5.2.7 B-15 成本披露报表（强制四列 + W1-W4 告警 + 净亏独立列）。
...
def add_report_subparsers(report_parser) -> None:
    ...
    # report context-effectiveness (V5.2.7 Context Effectiveness)
    ...
    # report cost-benefit（V5.2.7 B-15 成本披露报表）
    p_cb = sub.add_parser("cost-benefit", help="Cost-benefit disclosure report (V5.2.7 B-15)")
