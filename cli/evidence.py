# -*- coding: utf-8 -*-
"""V5.3.1 统一证据校验（Fourth Hardening P1-2：local_file 显式 schema）。

《V5.3.1 第四轮最终收敛修复任务》P1-2：第三轮 Evidence validator 仍接受
``none`` 与裸 hash（``sha256:...`` / 64 hex），导致"无真实证据的 PASS"可绕过。
本轮将证据语义收敛为唯一 schema：

    {"type": "local_file", "path": "<相对路径>", "sha256": "<64 hex>"}

V5.3.1 仅支持 local_file 类型。其余形式（external_uri / signed_attestation /
...
def parse_evidence(item: Union[str, Dict, None]) -> EvidenceResult:
    ...
    if isinstance(item, str):
        ...
        if raw.lower() == "none":
            return EvidenceResult(ok=False, error="evidence type 'none' is rejected in V5.3.1 (local_file schema only)")
        if raw.lower().startswith("sha256:") or _HEX64.fullmatch(raw.lower()):
            return EvidenceResult(ok=False, error="bare digest evidence is rejected in V5.3.1; provide a real local_file path")
        return _validate_local_file(raw, raw)
    if isinstance(item, dict):
        etype = str(item.get("type") or "").strip().lower()
        if etype not in _SUPPORTED_TYPES:
            return EvidenceResult(ok=False, error=f"evidence type {etype!r} is not supported in V5.3.1 (local_file only)")
