# -*- coding: utf-8 -*-
"""V5.3.0 高风险动作收据（execution receipt）命令。
...
V5.3.0 B-16（P1 补强，评审表 9.4 第 3 行 / Q2 / Q8）：
- capability 块新增 provenance（trusted/untrusted/unknown）与 delegated_from（V5.3.0 恒 null）
...
V5.3.0 9.5-1（P1 补强，评审表 9.5 第 1 行 / 升级计划 §3.1 L115-116）：
...
# --- V5.3.0 B-16：provenance 轻量判定规则（版本化 + 内容哈希双锚点） ---
...
def classify_provenance(resource_ref: str | None) -> tuple[str, str]:
    """V5.3.0 B-16 纯确定性判定器（R1-R4，fail-closed）。
...
def _build_capability(resource_ref: str | None, declared_provenance: str | None, declared_sensitivity: str | None) -> dict:
    """构造 receipt capability 块（V5.3.0 B-16）。
    ...
    # V5.3.0 9.5-1：敏感信息路径扫描（与 B-16 provenance/sensitivity 双轴正交）
    ...
    return {
        ...
        "delegated_from": None,  # V5.3.0 恒 null（schema 预留，升级计划 L219）
        "classifier_version": _CLASSIFIER_VERSION,
        "classifier_sha256": _CLASSIFIER_SHA256,
        "classification_status": status,
        # V5.3.0 9.5-1：敏感路径扫描结果（与 B-16 provenance/sensitivity 双轴正交，禁止合并）
        "scan": scan_result,
    }
...
def cmd_receipt(args) -> int:
    ...
    # V5.3.0 单一活动契约：旧契约非终态任务须先经官方 migrate/retire 处理；业务命令不直接在旧契约上追加收据。
    ...
    record = {
        ...
        # V5.3.0 B-16：capability 块（纯增量，不删旧字段）
...
def add_receipt_subparsers(subparsers) -> None:
    ...
    # V5.3.0 B-16：capability 判定输入（纯增量）
