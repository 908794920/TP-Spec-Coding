# -*- coding: utf-8 -*-
"""Read-only Evidence View projection for the local TP-Spec workbench.
...
def _candidate(
    *,
    ...
    return {
        ...
        "sources": [{**source, "sha256": str(sha256 or ""), "verification": verification}],
        "occurrence_count": 1,
    }
...
def build_evidence_view(
    ...
    *,
    ...
    for original in events:
        ...
        source_path = detail.get("source_evidence")
        if isinstance(source_path, str) and source_path.strip():
            add(_candidate(raw_path=source_path, declared_anchor="task", verification="structured",
                           task_id=str(task_id), task_dir=task_dir,
                           source=_source(event, "detail.source_evidence"),
                           sha256=str(detail.get("source_evidence_sha256") or "")))
...
def inspect_evidence_view(rows: List[Dict[str, Any]], *, task_dir: Path) -> List[Dict[str, Any]]:
    """Check file bytes once per anchored path, retaining each occurrence's hash.

    A match is file integrity only; current subject/role/AC scope is evaluated by
    the existing Runtime readers. Legacy anchors are not silently upgraded.
    """
    from cli.evidence import validate_evidence_path
    out = []
    for original in rows:
        row = {**original, "current_sha256": "", "inspection": "not_checked"}
        row["sources"] = [dict(source) for source in original.get("sources") or []]
        if row.get("anchor") == "task":
            try:
                checked = validate_evidence_path(task_dir, row["normalized_path"])
                row["current_sha256"] = checked.sha256
                row["inspection"] = "read" if checked.ok else "unavailable"
                if not checked.ok:
                    row["inspection_error"] = checked.error
            except (OSError, ValueError, UnicodeError) as exc:
                row.update(inspection="unavailable", inspection_error=str(exc))
        for source in row["sources"]:
            declared = source.get("sha256")
            source["integrity"] = ("not_checked" if row["inspection"] == "not_checked" else
                                   "unavailable" if row["inspection"] != "read" else
                                   "not_recorded" if not declared else
                                   "match" if declared == row["current_sha256"] else "mismatch")
        out.append(row)
    return out
