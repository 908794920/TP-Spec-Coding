# -*- coding: utf-8 -*-
"""V5.2.8 B-18 cutover 快照工具（T1，非破坏性）。
...
def add_cutover_snapshot_subparsers(subparsers) -> None:
    """注册 cutover-snapshot 子命令（V5.2.8 B-18 T1）。"""
    p = subparsers.add_parser(
        "cutover-snapshot",
        help="V5.2.8 B-18 T1: create a read-only base-repo snapshot + manifest + cutover receipt (non-destructive, no state change)",
    )
