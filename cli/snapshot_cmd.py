...
def add_cutover_snapshot_subparsers(subparsers) -> None:
    """注册 cutover-snapshot 子命令（B-18 T1）。"""
    p = subparsers.add_parser(
        "cutover-snapshot",
        help="B-18 T1: create a read-only base-repo snapshot + manifest + cutover receipt (non-destructive, no state change)",
    )
