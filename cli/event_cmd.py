...
def cmd_event_sync(args) -> int:
    ...
    V5.2.0 Hardening（任务书 §4.3）：默认禁止 event sync 推进权威状态。
...
def add_event_subparsers(event_parser) -> None:
    ...
    # event sync（M5-C：回流 flush 追加事件到 DB；V5.2.0 默认禁止状态推进）
    ...
    # V5.2.0 Hardening：显式管理员恢复模式（走共享 transition_service）
    p_sync.add_argument("--admin-recovery", action="store_true", help="V5.2.0 personal mode: explicit managed recovery through shared transition validation")
    ...
    p_sync.add_argument("--actor", required=False, default=None, help="V5.2.0: recovery actor (default human_owner)")
