...
def cmd_event_sync(args) -> int:
    ...
    V5.3.0 Hardening（任务书 §4.3）：默认禁止 event sync 推进权威状态。
        ...
        try:
            ...
            with dbmod.transactional(conn):
                conn.execute(
                    ...
                    "VALUES (?,?,?,?,?,?,?)",
                    (task_id, "AUDIT", "human_owner", "admin recovery", detail, "5.3.0", now),
                )
                conn.execute(
                    "INSERT INTO task_event (task_id,event_type,actor_role,summary,detail_json,workflow_version,created_at) "
                    "VALUES (?,?,?,?,?,?,?)",
                    (task_id, "RECONCILIATION", "human_owner", "admin recovery", detail, "5.3.0", now),
                )
        finally:
            conn.close()
        ...
        with dbmod.transactional(conn):
            for fid in new_flush_ids:
                for evt in groups[fid]:
                    ...
                    inserted += 1
        # V5.3.0：非状态 FACT 导入禁止 UPDATE task 表。
...
def add_event_subparsers(event_parser) -> None:
    ...
    # V5.3.0：显式管理员恢复模式（只走当前五态 record-first runtime）
    p_sync.add_argument("--admin-recovery", action="store_true", help="V5.3.0 personal mode: explicit managed recovery through current five-state runtime")
