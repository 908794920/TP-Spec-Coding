# -*- coding: utf-8 -*-
"""V5.2.5 B-18 cutover 回滚工具（T2，依赖 T1 快照）。
...
- 回滚后验证断言（§4.3 / §3.8 L258）：
  D1 目标版本（默认下一版本 5.2.5）新写入被拒（gate_task_contract → VERSION_MISMATCH）
...
def add_cutover_rollback_subparsers(subparsers) -> None:
    """注册 cutover-rollback 子命令（V5.2.5 B-18 T2）。"""
    p = subparsers.add_parser(
        "cutover-rollback",
        help="V5.2.5 B-18 T2: restore base repo from snapshot + §4.3 rollback assertions (default dry-run zero-write)",
    )
