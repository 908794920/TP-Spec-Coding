...
Work 范围、失败/重试、真实结果、接收及候选加入 `tp-spec.task-learning/v1` 的 Task 学习输入；认领本身不当成知识。来源按已知 Task 文件/事件定向读取，文件片段不当成磁盘文件名。每个 Work 给紧凑结果，由父 Task 统一提炼，不为每项重复全量知识收口或生成四份总结。
