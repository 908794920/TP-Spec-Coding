...
软件领域的 active formal Role 由 `governance/role-catalog.yaml` 统一登记；Role Skill 位于 `skills/roles/`，共享能力位于 `skills/capabilities/`，自治专项能力位于 `skills/autonomy/`。旧 action-role 只存在于 previous active contract → 5.2.7 migration/history，不参与 active routing。
