...
日期：2026-09-26。范围仅为 R01 文档口径纠偏与 R02 用户级外部自定义 SKILL；不是正式上线或整版验收结论。

### 用户级外部 SKILL

- 新增共享能力目录与 `skill list/read`：标准 `SKILL.md` 自动发现、非标准 Markdown 登记、来源独立身份、状态与精确正文读取。不将外部资产加入内置 Catalog、Manifest 或业务项目。
- `/tp-spec-coding` 与领域指引按需发现、读取并传递来源；工作台后端与 CLI 共用目录/正文读取，界面在图、树及共享详情中显示外部来源、状态、指纹和未关联条目。
- 详情支持来源内相对 Markdown、锚点、返回和手动重新读取；声明关系不冒充调用统计，停用内容只允许显式人工检查，不构成执行授权。
- 活动元数据和模板统一为 `5.3.5`；将本次输入基线纳入既有显式迁移来源，未改变迁移步骤、Task 公共状态或 Runtime schema。管理员恢复审计改用现有 `active_version()`，不再写死旧活动版本。

### 本版验证边界

按影响执行临时局部检查、发布元数据/文档维护和补丁重放，不恢复历史测试套件或默认全量回归。前端锁定依赖在本轮云端因 DNS `EAI_AGAIN` 无法完整取得；完整应用类型检查未通过、构建未完成、浏览器交互未执行。Windows 原生路径、现场迁移、宿主入口采用及真实业务任务由用户自行验证；具体命令、结果和检查步骤见随附最终交付文档。历史章节中的通过或失败只属于原批次，不能覆盖本版结果。


### 文档口径纠偏

- 当前文档补齐已接入的知识库页面、只读 API 和能力来源说明，区分产品入口与领域 Agent；生成目录继续以 Role Catalog 为准。
- 安装说明不再绑定某次历史 ZIP/补丁链；Runtime 与 Knowledge 投影的 schema 边界分开。活动模板与收敛指引按实际契约/预检解释，不再用 `DEFERRED` 或补丁批次名称推断结单条件。
- 将需保留的历史引用处置、前端及 Knowledge 验证记录归位到相应版本历史；保留原日期/输入/范围/失败与未执行项，不为缺少日期的记录补造日期。文档纠偏本身不改变 Runtime 门禁或业务行为，也不构成新一轮产品验收；本版活动版本在收尾时单独统一。
...
<a id="history-source-patch-chain"></a>

### 历史源码补丁链说明（原记录未单独注明日期）

以下是原升级手册对应交付的适用链，不是当前补丁操作指令；当前用法见 [Getting Started](docs/GETTING_STARTED.md#10-同版本源码升级与兼容)。
...
<a id="history-capability-extraction"></a>

### 能力方法抽取与引用处置（原记录未单独注明日期）

以下从原能力模型说明归位，只描述当期源码范围，不证明用户安装或宿主缓存已更新：
...
<a id="history-workbench-p6"></a>

### P6 与 Work 详情的当期验证边界（原记录未单独注明日期）

以下从原工作台说明归位；其中“本批”和“云端”仅指该记录对应批次：
...
<a id="history-task-mainline"></a>

### 任务主线展示验证（原记录未注明日期）

以下保留原主线改版的检查范围，不追溯改写更早批次的限制：
...
<a id="history-workbench-visual"></a>

### 工作台外观与交互验证 — 2026-09-21

以下保留原本地视觉批次结果，不覆盖后来新增页面或所有执行语义：
...
<a id="history-knowledge-usage"></a>

### Knowledge 检索、读取与使用分析批次（原记录未注明日期）

以下从 `docs/KNOWLEDGE_USAGE.md` 原第 7 节归位；“本轮”只指基于 `0ea0703` 快照的这次历史实现，不是当前文档修正或整版验收。当前用法与现场检查边界见 [Knowledge 使用说明](docs/KNOWLEDGE_USAGE.md)。
...
<a id="history-workbench-early"></a>

### 早期工作台验证边界（W02–W05；原记录未单独注明日期）

以下从原工作台说明归位，保持当期环境与待验状态；后续检查另见各版本/批次记录，不以此否定后来已经发生的局部验证：
