...
1. 先读取用户 TP-Spec-Coding Installation（默认 `~/.tp-spec/installation.yaml`，或 `TP_SPEC_INSTALLATION_CONFIG`），解析 physical Base Root、Knowledge System Root；项目绑定/Registry 再解析当前 Knowledge Project Root。
2. 读取当前 Base 的 `automation/knowledge/daily-maintenance.md`，严格按该版本协议执行。
3. 使用 `tp-knowledge` 与正式 `tp-spec knowledge ...` CLI；不得依赖项目 `.tp-spec/knowledge` / `.tp-spec/scripts` Junction，也不要调用 Knowledge Vault 中的 legacy tools。
