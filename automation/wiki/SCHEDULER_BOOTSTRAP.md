...
1. 先读取用户 TP-Spec-Coding Installation（默认 `~/.tp-spec/installation.yaml`，或 `TP_SPEC_INSTALLATION_CONFIG`），解析 physical Base Root 与 Wiki System Root；不得在 Scheduler Prompt 中硬编码用户机器绝对路径。
2. 从当前 Base 读取 `automation/wiki/daily-maintenance.md`，严格按该版本 canonical protocol 执行。
3. 使用正式 `<BaseRoot>/scripts/tp-spec.ps1`（或等价已安装 `tp-spec` 命令）；不得依赖项目 `.tp-spec/scripts` / `.tp-spec/wiki` Junction，也不得调用旧 Wiki 数据仓中的 legacy tools。
