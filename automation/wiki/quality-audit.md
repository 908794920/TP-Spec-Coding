...
1. 先 `wiki status --repo <id>` 核对已有固定来源；独立审计使用成功 baseline，有完整 pending 时明确审计该 candidate，不混用当前工作区。运行 `wiki manifest-refresh`（只刷新机器事实，不改变正文语义）。
...
4. 执行 `wiki audit --full --repo <id>` 获取确定性**全仓 mandatory scope**；逐篇深读计划内全部 durable 文档并用 `wiki source-read --repo <id> --path <source>` 回到该固定来源核验 cite 与正文结论。`--full` 不得用抽样替代。
