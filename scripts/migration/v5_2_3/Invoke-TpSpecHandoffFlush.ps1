<#
.SYNOPSIS
Atomically flushes handoff.json into task facts and verifiable generated views (V5.2.3 single contract).

.DESCRIPTION
The flush follows prepare -> validate -> commit -> consume. It operates only on
artifact_contract.version=5.2.3 tasks with tool-agnostic tp-* / human_owner actors;
...
function Get-GeneratedSources {
    ...
    foreach ($relative in $required) {
        ...
        if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
            throw "V5.2.3 generated view requires '$relative' before flush."
        }
...
try {
    ...
    # V5.2.3 单一活动契约：旧契约非终态任务须先经官方 migrate/retire；flush 不负责跨契约迁移。
    ...
    if ($ReviewOnly) {
        if ($Actor -ne 'tp-verification-engineering') { throw 'V5.2.3 -ReviewOnly is only available to tp-verification-engineering.' }
        $stateBefore = Get-YamlScalar -Text $statusBefore -Name 'current_state'
        if ($stateBefore -ne 'VERIFYING') { throw "V5.2.3 -ReviewOnly requires current_state=VERIFYING (got '$stateBefore')." }
        ...
        if ($null -eq $review -or $review.Actor -ne 'tp-verification-engineering' -or $review.Decision -notin @('PASS', 'FAIL', 'NEEDS_FIX') -or [string]::IsNullOrWhiteSpace($review.Evidence)) {
            throw 'V5.2.3 -ReviewOnly requires valid codex-review.md review metadata.'
        }
        $reviewTime = [DateTimeOffset]::MinValue
        if (-not [DateTimeOffset]::TryParse([string]$review.Timestamp, [ref]$reviewTime)) { throw 'V5.2.3 -ReviewOnly requires codex-review.md review.timestamp in ISO 8601 format.' }
    ...
    if ([string]$handoff.schema_version -ne $script:ActiveVersion) { throw "V5.2.3 handoff.json.schema_version must be $($script:ActiveVersion)." }
    ...
    # V5.2.3 next_prompt 完整性与目标一致性校验。
    if ($null -eq $handoff.next_prompt) { throw 'V5.2.3 handoff.json.next_prompt is required.' }
    foreach ($field in @('target_role', 'invocation', 'task_id', 'target_state', 'risk_level', 'page_verification', 'entry', 'reading_order', 'actions', 'constraints', 'exit_expectation', 'fact_source_disclaimer')) {
        if (-not ($handoff.next_prompt.PSObject.Properties.Name -contains $field) -or $null -eq $handoff.next_prompt.$field -or
            ...
            throw "V5.2.3 handoff.json.next_prompt.$field is required."
        }
    }
    if ([string]$handoff.next_prompt.target_role -ne [string]$handoff.next.owner) { throw 'V5.2.3 next_prompt.target_role must equal next.owner.' }
    if ([string]$handoff.next_prompt.target_state -ne [string]$handoff.next.state) { throw 'V5.2.3 next_prompt.target_state must equal next.state.' }
    if ([string]$handoff.next_prompt.task_id -ne $task.Name) { throw 'V5.2.3 next_prompt.task_id must equal task directory name.' }
    if ([string]$handoff.next_prompt.risk_level -ne (Get-YamlScalar -Text $statusBefore -Name 'risk_level')) { throw 'V5.2.3 next_prompt.risk_level must equal status.yaml.risk_level.' }
    if ([string]$handoff.next_prompt.entry -ne 'generated/continuation.md') { throw 'V5.2.3 next_prompt.entry must be generated/continuation.md.' }
    ...
    $expectedNextOwner = if ([string]$handoff.next.state -eq 'COMPLETED') {
        # V5.2.3：结项由 tp-delivery-convergence 从 CLOSING 提交。
    ...
    if (-not [string]::IsNullOrWhiteSpace($expectedNextOwner) -and [string]$handoff.next_prompt.target_role -ne $expectedNextOwner) { throw "V5.2.3 next_prompt.target_role must equal owner '$expectedNextOwner' of next state '$($handoff.next.state)'." }
    ...
    # SHD M3 转换合法性 + M2 flush 侧就绪声明校验（V5.2.3）
    $stateBefore = Get-YamlScalar -Text $statusBefore -Name 'current_state'
    $flushTransitions = @{
    ...
    if ($flushTransitions.ContainsKey($stateBefore) -and [string]$handoff.next.state -notin $flushTransitions[$stateBefore]) {
        throw "V5.2.3 handoff.next.state '$($handoff.next.state)' is not a valid transition from '$stateBefore'."
    }
    ...
    if ($flushShdDriver.ContainsKey($stateBefore) -and $flushForwardSuccessors.ContainsKey($stateBefore) -and [string]$handoff.next.state -in $flushForwardSuccessors[$stateBefore]) {
        ...
        if ($null -eq $shd) {
            throw "V5.2.3 state '$stateBefore' requires stage_handoff declaration in '$($shdDriver.artifact)' before flush."
        }
        if ($shdDriver.review) {
            if ($shd.Decision -ne 'PASS') { throw "V5.2.3 state '$stateBefore' requires codex-review.md review.decision=PASS before flush (got '$($shd.Decision)')." }
            if ([string]::IsNullOrWhiteSpace($shd.NextState)) { throw "V5.2.3 codex-review.md review.next_state is required before flush." }
            if ([string]$shd.NextState -ne [string]$handoff.next.state) { throw "V5.2.3 codex-review.md review.next_state '$($shd.NextState)' must equal handoff.next.state '$($handoff.next.state)'." }
        }
        else {
            if ($shd.Status -ne 'ready') { throw "V5.2.3 state '$stateBefore' requires $($shdDriver.artifact) stage_handoff.status=ready before flush (got '$($shd.Status)')." }
            if ([string]::IsNullOrWhiteSpace($shd.IntendedNext)) { throw "V5.2.3 $($shdDriver.artifact) stage_handoff.intended_next is required before flush." }
            if ([string]$shd.IntendedNext -ne [string]$handoff.next.state) { throw "V5.2.3 $($shdDriver.artifact) stage_handoff.intended_next '$($shd.IntendedNext)' must equal handoff.next.state '$($handoff.next.state)'." }
        }
    }

    # V5.2.3 结单链门控（与 tp-spec commit / task transition 一致）。
    $nextState = [string]$handoff.next.state
    $nextOwner = [string]$handoff.next.owner
    if ($nextState -eq 'COMPLETED') {
        if ($stateBefore -ne 'CLOSING') { throw 'V5.2.3 completion must come from CLOSING; VERIFYING cannot reach COMPLETED directly.' }
        if ($Actor -ne 'tp-delivery-convergence' -or $nextOwner -ne 'tp-delivery-convergence') { throw 'V5.2.3 COMPLETED must be flushed by tp-delivery-convergence.' }
    }
    elseif ($nextState -eq 'CLOSING') {
        if ($Actor -ne 'tp-delivery-convergence' -or $nextOwner -ne 'tp-delivery-convergence') { throw 'V5.2.3 CLOSING may only be entered by tp-delivery-convergence.' }
        ...
        if (-not $hasReviewPass) { throw 'V5.2.3 CLOSING requires an existing tp-verification-engineering REVIEW_COMPLETED=PASS event.' }
    }
