# -*- coding: utf-8 -*-
"""V5.2.5 Windows portability and execution-boundary regression tests."""
    ...
    try:
        ...
        with dbmod.transactional(conn):
            conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (project_id, project_id, str(root_path), "5.2.5", dbmod.EXPECTED_SCHEMA_VERSION, now, now),
            )
    finally:
        conn.close()
...
def test_legacy_registry_migration_treats_alias_roots_as_same_identity(tmp_path: Path):
    ...
    _write(
        ...
        json.dumps({"projects": [{
            ...
            "base_version": "5.2.5", "schema_version": 1,
        }]}),
    )
    _write(
        current,
