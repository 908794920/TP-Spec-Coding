...
def test_l1_standard_route():
    with tempfile.TemporaryDirectory() as td:
        ...
        r=orchestration.resolve_route('TASK-V514',db_path=db)
        assert r['next_stage']=='development' and r['role_id']=='tp-development-engineer'
        assert r['transition_notice_required'] is True and r['transition_from_role']=='tp-software-architect'
...
def test_l3_architecture_review_then_material_confirmation():
    with tempfile.TemporaryDirectory() as td:
        ...
        r=orchestration.resolve_route('TASK-V514',db_path=db)
        assert r['next_stage']=='planning' and r['role_id']=='tp-tech-lead'
        ...
        assert r['transition_from_role']=='tp-tech-lead'
        add_workflow_confirmation(db,'TASK-V514')
        r=orchestration.resolve_route('TASK-V514',db_path=db)
        assert r['confirmation_required'] is False and r['skill_path'].endswith('tp-development-engineer/SKILL.md')

def test_verification_rework_and_deep_review_are_separate_roles():
    with tempfile.TemporaryDirectory() as td:
        db=make_db(Path(td)/'a.db',risk='L3',flow='L3')
        for actor,phase in [('tp-product-manager','requirement'),('tp-software-architect','architecture')]: add_checkpoint(db,'TASK-V514',actor,phase)
        add_review(db,'TASK-V514','PASS')
        add_checkpoint(db,'TASK-V514','tp-tech-lead','planning')
        add_workflow_confirmation(db,'TASK-V514')
        add_checkpoint(db,'TASK-V514','tp-development-engineer','development')
        r=orchestration.resolve_route('TASK-V514',db_path=db)
        assert r['next_stage']=='verification' and r['role_id']=='tp-test-engineer' and r['execution_mode']=='DIRECT'
        assert r['transition_from_role']=='tp-development-engineer'
        add_verify(db,'TASK-V514','NEEDS_FIX')
        r=orchestration.resolve_route('TASK-V514',db_path=db)
        assert r['next_stage']=='development' and 'VERIFICATION_NEEDS_FIX' in r['reason_codes']
        assert r['transition_from_role']=='tp-test-engineer'

    with tempfile.TemporaryDirectory() as td:
        db=make_db(Path(td)/'b.db',risk='L3',flow='L3')
        for actor,phase in [('tp-product-manager','requirement'),('tp-software-architect','architecture')]: add_checkpoint(db,'TASK-V514',actor,phase)
        add_review(db,'TASK-V514','PASS')
        add_checkpoint(db,'TASK-V514','tp-tech-lead','planning')
        add_workflow_confirmation(db,'TASK-V514')
        ...
        r=orchestration.resolve_route('TASK-V514',db_path=db)
        assert r['next_stage']=='review' and r['role_id']=='tp-code-reviewer' and r['execution_mode']=='DEEP_REVIEW'
