...
def test_active_contract_cutover_is_complete():
    ...
    status=yaml.safe_load((BASE/'templates/5.2.4/status.yaml').read_text(encoding='utf-8'))
    assert status['artifact_contract']['version']=='5.2.4'
    assert status['base_version']=='5.2.4'
...
def test_compat_matrix_has_only_active_contract():
    ...
    assert set(c['contracts'])=={'5.2.4'}
    assert c['contracts']['5.2.4']['status_contract']=='5.2.4'
    assert c['contracts']['5.2.4']['governance_ranges']['orchestration']=='[5.2.4, 5.3.0)'
