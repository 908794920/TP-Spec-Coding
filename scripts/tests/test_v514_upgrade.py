...
def test_active_contract_cutover_is_complete():
    ...
    assert (BASE/'templates/5.2.1/status.yaml').is_file()
    assert not (BASE/'templates'/('5.1.' + '3')).exists()
    status=yaml.safe_load((BASE/'templates/5.2.1/status.yaml').read_text(encoding='utf-8'))
    assert status['artifact_contract']['version']=='5.2.1'
    assert status['base_version']=='5.2.1'
...
def test_compat_matrix_has_only_active_contract():
    ...
    assert set(c['contracts'])=={'5.2.1'}
    assert c['contracts']['5.2.1']['status_contract']=='5.2.1'
    assert c['contracts']['5.2.1']['governance_ranges']['orchestration']=='[5.2.1, 5.3.0)'
