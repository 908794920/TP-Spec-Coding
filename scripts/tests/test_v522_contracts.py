...
def test_boundary_confirmation_is_bound_to_source_fact_and_target():
    ...
    event={'task_id':'TASK-1','event_type':'WORKFLOW_CONFIRMATION','actor_role':'human_owner','created_at':'2026-08-14T00:00:00Z','workflow_version':'5.3.0','detail_json':json.dumps({'producer':'workflow_confirm','transaction_id':'tx1','schema_version':'5.3.0','task_id':'TASK-1','actor_role':'human_owner','created_at':'2026-08-14T00:00:00Z',**binding})}
...
def test_confirmation_detail_keeps_binding_and_trusted_identity_fields():
    ...
    detail=build_confirmation_detail(task_id='TASK-1',binding=binding,transaction_id='tx-1',flush_id='CONFIRM-1',created_at='2026-08-14T00:00:00Z',schema_version='5.3.0')
...
def test_delivery_detail_binds_verification_and_readiness_fields():
    detail=build_delivery_detail(
        ...
        created_at='2026-08-14T00:00:00Z', schema_version='5.3.0',
...
def _trusted_event(event_id, event_type, actor, producer, detail_extra=None):
    detail={'transaction_id':f'tx-{event_id}','producer':producer,'schema_version':'5.3.0','task_id':'TASK-1','actor_role':actor,'created_at':'2026-08-14T00:00:00Z'}
    detail.update(detail_extra or {})
    return {'id':event_id,'task_id':'TASK-1','event_type':event_type,'actor_role':actor,'to_stage':'','summary':'','detail_json':json.dumps(detail,ensure_ascii=False),'workflow_version':'5.3.0','created_at':'2026-08-14T00:00:00Z'}
