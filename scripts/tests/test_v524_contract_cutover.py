...
def test_active_contract_is_only_524():
    assert (BASE / "VERSION").read_text(encoding="utf-8").strip() == "5.2.8"
    compat = yaml.safe_load((BASE / "governance/compat-matrix.yaml").read_text(encoding="utf-8"))
    assert set(compat["contracts"]) == {"5.2.8"}
    assert compat["contracts"]["5.2.8"]["base_version"] == "5.2.8"
...
def test_governed_role_first_contract_versions_are_524():
    for rel, field in [
        ...
        assert str(data[field]) == "5.2.8", rel
...
def test_only_active_template_contract_is_524():
    ...
    assert version_dirs == ["5.2.8"]
    assert (templates / "5.2.8/requirement.md").is_file()
    assert not (templates / "5.2.8/requirement-knowledge.md").exists()
