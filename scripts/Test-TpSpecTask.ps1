...
.DESCRIPTION
V5.2.3 single-contract validator. Public Record-first tasks are checked for
...
not V5.2.3 progression gates. Legacy microstate checks remain below only for
...
# ==================================================
# V5.2.3 P3: governance owner/transition values now come from the controlled loader (C-01.4/D-06)
# ==================================================
...
function Test-GeneratedSourcesContract {
    ...
    # V5.2.3 §3.8/§10.2：五个新工件经集中注册表纳入 source digest（存在才纳入），
    ...
    if (($actual -join "`n") -ne ($want -join "`n")) {
        Add-Issue Error 'GENERATED_SOURCE_SET_MISMATCH' "V5.2.3 generated view '$RelativePath' must cover the formal artifacts completed before state '$CurrentState'."
    }
}

function Get-AutomatedRiskRules {
    # V5.2.3 P3/C-02: read risk-rule.yaml (automated_validation) through the controlled
...
# V5.2.3 单一活动契约：旧契约非终态任务须先经官方 migrate/retire；本验证入口不代替跨契约迁移。
if (-not [string]::IsNullOrWhiteSpace($statusText) -and [string]$artifactContractVersion -ne $script:ActiveVersion) {
    Write-Output 'TP-Spec-Coding task validation summary (V5.2.3 single active contract)'
    ...
    Write-Output "[Error] LEGACY_CONTRACT_REJECTED - artifact_contract.version '$artifactContractVersion' is a frozen legacy contract; the V5.2.3 runtime validates only $($script:ActiveVersion). Historical tasks are static audit archives kept in Git release branches."
    exit 1
}

# V5.2.3 Record-first fast path. The PowerShell wrapper validates identity/text basics
...
if (-not [string]::IsNullOrWhiteSpace($statusText) -and [string]$artifactContractVersion -eq $script:ActiveVersion -and
    ...
    Write-Output 'TP-Spec-Coding task validation summary (V5.2.3 Record-first integrity)'
    Write-Output "Task directory: $($taskDirectory.FullName)"
    Write-Output "Task id: $taskId"
    Write-Output "Current state: $currentState"
    Write-Output "Current owner: $currentOwner"
...
# V5.2.3 单一活动契约：入口已拒绝旧契约，以下门控均按 5.2.3 语义恒定。
...
# ==================================================
# V5.2.3 Hardening（审查报告 P0-2/P0-3/P1-3/P1-5）：
...
if ($requiresArtifactContract) {
    ...
    if ($hardenL23 -and $currentState -in @('DEVELOPING', 'VERIFYING', 'BROWSER_VERIFYING', 'REVIEWING', 'CLOSING', 'COMPLETED')) {
        foreach ($name in $hardenNewArtifacts) {
            ...
            if (-not (Test-Path -LiteralPath $p -PathType Leaf)) {
                Add-Issue Error 'HARDEN_ARTIFACT_MISSING' "V5.2.3 L2/L3 requires new artifact '$name' for state '$currentState'."
            }
        }
    }
    ...
    if ($hardenL23 -and $currentState -in @('DEVELOPING', 'VERIFYING', 'BROWSER_VERIFYING', 'REVIEWING', 'CLOSING', 'COMPLETED')) {
        ...
        if (Test-Path -LiteralPath $archReviewPath -PathType Leaf) {
            ...
            if ($null -ne $archFront) {
                ...
                if ($archDecision -ne 'PASS') {
                    Add-Issue Error 'HARDEN_ARCH_REVIEW_REQUIRED' "V5.2.3 L2/L3 architecture-review.md review.decision must be PASS, got '$archDecision'."
                }
            }
            else {
                Add-Issue Error 'HARDEN_ARCH_REVIEW_REQUIRED' 'V5.2.3 L2/L3 architecture-review.md is missing structured review front matter.'
            }
        }
        ...
        if (-not $archEventFound) {
            Add-Issue Error 'HARDEN_ARCH_REVIEW_REQUIRED' "V5.2.3 L2/L3 requires a tp-architecture-review REVIEW_COMPLETED PASS event for state '$currentState'."
        }
    }
    ...
    if ($currentState -in @('REVIEWING', 'CLOSING', 'COMPLETED')) {
        ...
        if (Test-Path -LiteralPath $tgPath -PathType Leaf) {
            ...
            if ($null -ne $tgFront) {
                ...
                if ($tgVerification -notin @('done', 'completed', 'complete')) {
                    Add-Issue Error 'HARDEN_TEST_GUIDE_INCOMPLETE' "V5.2.3 closing state '$currentState' requires requirement-test-guide.md lifecycle.verification_results done."
                }
            }
        }
        ...
        if (Test-Path -LiteralPath $crPath -PathType Leaf) {
            ...
            if ($crBody -notmatch '结论' -or $crBody -notmatch '证据' -or $crBody -notmatch '残余风险') {
                Add-Issue Error 'HARDEN_CODE_REVIEW_EMPTY' 'V5.2.3 closing requires codex-review.md body containing 结论/证据/残余风险 sections.'
            }
        }
    }
}
        ...
        if (Test-Path -LiteralPath $generatedDirectory -PathType Container) {
            ...
            Get-ChildItem -LiteralPath $generatedDirectory -Force -Recurse | ForEach-Object {
                ...
                if ($_.PSIsContainer -or $relative -notin $allowedGenerated) {
                    Add-Issue Error 'GENERATED_UNMANAGED_FILE' "V5.2.3 generated directory may contain only generated continuation/final-result views; found '$relative'."
                }
            }
        }
    }
    ...
    if (Test-Path -LiteralPath $acceptancePath -PathType Leaf) {
        ...
        if ($acceptanceText -match '(?m)^\s*action:\s*["'']?DML["'']?\s*(?:#.*)?$') {
            ...
            if (-not $hasDmlEvidence -and -not $riskAccepted) {
                Add-Issue Error 'DML_EVIDENCE_MISSING' 'V5.2.3 DML acceptance requires execution evidence, result verification and rollback/cleanup; EXPLAIN or read-only evidence is insufficient.'
            }
        }
    }

    # V5.2.3 personal mode：COMPLETED 必须经过 tp-delivery-convergence CLOSING；不要求人员 APPROVE。
    if ($currentState -eq 'COMPLETED') {
        if ($currentOwner -ne 'tp-delivery-convergence') {
            Add-Issue Error 'COMPLETION_OWNER_INVALID' "V5.2.3 COMPLETED must be owned by tp-delivery-convergence, not '$currentOwner'."
        }
        if (-not $script:VisitedStates.ContainsKey('CLOSING')) {
            Add-Issue Error 'CLOSING_CHAIN_MISSING' 'V5.2.3 completion requires a CLOSING STATE event committed by tp-delivery-convergence.'
        }
    }
...
if (Test-Path -LiteralPath $acceptancePath -PathType Leaf) {
    ...
    # 人工页面验收声明（page_verification 块）。V5.2.3 枚举：NOT_REQUIRED | human | verification | architecture（缺省视为无需页面验证）。
    $pageMode = ''
    $pageWitness = ''
    $pageWitnessEvidence = ''
    if ($requiresClosingChain) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($pageMode) -and $pageMode -notin @('NOT_REQUIRED', 'human', 'verification', 'architecture')) {
            Add-Issue Error 'PAGE_MODE_INVALID' "page_verification.mode '$pageMode' is invalid; V5.2.3 allows NOT_REQUIRED, human, verification, or architecture (or omit the block)."
        }
    ...
    if ($requiresClosingChain) {
        ...
        if ($null -eq $headerLine) {
            ...
            if ($headerCells.Count -ne 10 -or -not $headerLine.Trim().StartsWith('|') -or -not $headerLine.Trim().EndsWith('|') -or
                ...
                Add-Issue Error 'ACCEPTANCE_HEADER_INVALID' 'V5.2.3 acceptance matrix must use the standard eight-column header ending with the verdict column.'
            }
        }
    }
...
Write-Output 'TP-Spec-Coding task validation summary (V5.2.3 single active contract: artifacts, SHD closure, acceptance closure, receipts, closing chain)'
Write-Output "Task directory: $($taskDirectory.FullName)"
Write-Output "Task id: $taskId"
Write-Output "Current state: $currentState"
Write-Output "Current owner: $currentOwner"
